...
현재 `schema.prisma` (2026-05-15 기준) 모델 35개·enum 22개를 도메인 묶음으로 정리. 모든 모델은 `id String @id @default(uuid()) @db.Uuid` 기본. 본문은 핵심 필드/관계 위주.
...
erDiagram
    ...
    %% === 신고 ===
    User ||--o{ Report : "reports (as client)"
    User ||--o{ Report : "reported (as expert)"
...
**다이어그램에 없는 모델** (관계 없음): `Banner`, `Faq`, `MainSetting`. `MainSetting`은 `targetUserId?`/`targetServiceId?`로 User/Service를 가리키지만 Prisma FK가 아닌 다형 참조라 ER에서 생략.
...
알림               Notification
배너·메인·FAQ      Banner, MainSetting, Faq
...
- 상태: `isBlocked`, `isDeleted`, `deletedAt?`
- 부가: `profileImageUrl?`, `region?`, `bankName?`, `bankAccount?`, `phoneNumber?`
- 제약: `@@unique([provider, providerId])` — 소셜 가입 시 동일 provider+id 중복 방지
- 관계 (수가 많음): clientProfile/expertProfile, services(전문가), reviews, orders(client/expert 양방향), payments, refunds, favorites, recentlyViewed, reports, communityPosts, comments, likes, chatRooms, messages, notifications 등

### `Admin`
- 분리된 별도 테이블. `User`와 관계 없음 (관리자는 별도 인증)
- 핵심: `isSuper`, `mustChangePassword` (기본 true — 첫 로그인 시 강제 변경 의도), `lastLoginAt?`
- 관계: `csChatRooms`(담당), `sentCsMessages`, `approvedRefunds`, `communityPosts`(어드민 작성 가능)
...
- `userId @unique`
- 승인: `isApproved`, `approvedAt?`, `rejectReason?`
...
- 관계: `specialtyCategories`, `techStacks`, `portfolios`
...
- 관계: techStacks, images, steps, faqs, orders, favorites, chatRooms, recentlyViewedBy
...
- 상태: `status: OrderStatus` (13가지 — NEGOTIATING ~ REFUND_COMPLETED, 전체 라이프사이클)
- 환불: `refundReason?`
- 관계: `payment: Payment?`, `review: Review?`

### `Payment` (Order 1:1)
- `orderId @unique`
- 금액: `paidAmount`
- 상태: `status: PaymentStatus` (PENDING/PAID/FAILED/CANCELLED/REFUNDED)
- 외부 PG: `method: String`, `paymentKey? @unique`, `rawData: Json?` (PG 원본 응답 저장)
...
### `Report` (client → expert)
- `clientUserId`, `expertUserId` (User self-relation 2개)
- `reason`, `detail`
- 관계: `images: ReportImage[]`
...
- 내용: `title`, `content`, `likes: Int @default(0)` (집계 캐시)
...
### `Notification`
- `userId` (수신자)
- 분류: `type: NotificationType` (COMMUNITY/TRANSACTION/REMINDER), `category: NotificationCategory` (세부)
- 본문: `content: String`
...
- `targetUserId?` 또는 `targetServiceId?` (둘 중 하나만 사용)
### `Faq` — `title`, `content`
...
- 시스템 메시지: `systemType?: SystemMessageType` (거래 요청/결제/일정 등 9가지)
...
### `StatisticsBySeller`
- `sellerUserId @unique` — 전문가별 1행
- `totalTransactionAmount`, `totalTransactionCount`, `maxTransactionAmount`
- 실시간 계산 비용을 피하기 위한 캐시. 거래 발생 시 업데이트 필요(현재 코드엔 갱신 로직 미구현).

### `StatisticsByCategory`
- `(serviceGroupId, serviceCategoryId) @@unique` — 카테고리별 1행
- 동일 집계 필드
...
| `OrderStatus` | 13개 (NEGOTIATING ~ REFUND_COMPLETED) | Order 라이프사이클 |
...
- 다형 참조: `referenceType` + `referenceId` 쌍 (Notification, Message) — Prisma FK가 아니라 애플리케이션 레벨 라우팅용
