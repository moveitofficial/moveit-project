...
export class OAuthController {
  ...
  @Get('kakao')
  @UseGuards(KakaoOAuthStartGuard)
  @ApiOperation({
    summary: '카카오 OAuth 로그인 시작',
    description:
      'Kakao 인증 페이지로 리다이렉트합니다. role은 이 단계에서 받지 않습니다.',
  })
  @ApiResponse({
    status: HttpStatus.FOUND,
    description: 'Kakao OAuth 인증 페이지로 리다이렉트',
  })
  kakaoStart(): void {
    // Guard가 Kakao authorize URL로 리다이렉트
  }
  ...
  @Get('naver')
  @UseGuards(NaverOAuthStartGuard)
  @ApiOperation({
    summary: '네이버 OAUTH 로그인 시작',
    description:
      'Naver 인증 페이지로 리다이렉트합니다. role은 이 단계에서 받지 않습니다.',
  })
  @ApiResponse({
    status: HttpStatus.FOUND,
