...
class Seeder {
  ...
  async #seedOrdersAndPayments(
    ...
    const orderStatusPlan: OrderStatus[] = [
      ...Array.from({ length: 8 }).fill(OrderStatus.NEGOTIATING),
      ...Array.from({ length: 12 }).fill(OrderStatus.IN_PROGRESS),
      ...Array.from({ length: 6 }).fill(OrderStatus.DEADLINE_IMMINENT),
      ...Array.from({ length: 6 }).fill(OrderStatus.WORK_COMPLETED),
      ...Array.from({ length: 5 }).fill(OrderStatus.PURCHASE_CONFIRMED),
      ...Array.from({ length: 12 }).fill(OrderStatus.SETTLEMENT_REQUESTED),
      ...Array.from({ length: 6 }).fill(OrderStatus.SETTLEMENT_COMPLETED),
      ...Array.from({ length: 5 }).fill(OrderStatus.REFUND_REQUESTED),
    ];

    for (const element of orderStatusPlan) {
  ...
  async #seedCsChatRooms(clients: User[], admin: Admin): Promise<void> {
    ...
    const statusPlan: CsChatStatus[] = [
      ...Array.from({ length: 12 }).fill(CsChatStatus.OPEN),
      ...Array.from({ length: 8 }).fill(CsChatStatus.ASSIGNED),
      ...Array.from({ length: 5 }).fill(CsChatStatus.CLOSED),
    ];

    for (const element of statusPlan) {
      const client = pick(clients);
      const status = element;
